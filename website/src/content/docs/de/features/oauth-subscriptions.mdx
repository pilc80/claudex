...
Kein `claudex-config auth login`-Schritt erforderlich. Wenn Sie bereits bei Claude Code angemeldet sind, funktioniert es sofort.
...
1. `claudex-config auth login github` zeigt einen Device Code an
...
Liest Anmeldedaten aus der Gemini CLI-Konfiguration. Zuerst Gemini CLI installieren und authentifizieren, dann liest `claudex-config auth login google` das gespeicherte Token.
