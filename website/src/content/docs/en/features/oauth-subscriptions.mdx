...
No `claudex-config auth login` step is needed. If you are already logged in to Claude Code, it works immediately.
...
1. `claudex-config auth login github` displays a device code
...
Reads credentials from the Gemini CLI configuration. Install and authenticate with the Gemini CLI first, then `claudex-config auth login google` reads the stored token.
