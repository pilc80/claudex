...
<Aside type="note">
  Claude OAuth profiles use a special gateway auth mode. Claudex skips the proxy and lets Claude Code use its own OAuth directly. No separate `claudex-config auth login` step is needed.
</Aside>
