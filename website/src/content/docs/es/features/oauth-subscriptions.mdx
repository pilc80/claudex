...
No se necesita paso de `claudex-config auth login`. Si ya tienes sesion iniciada en Claude Code, funciona inmediatamente.
...
1. `claudex-config auth login github` muestra un codigo de dispositivo
...
Lee credenciales de la configuracion de Gemini CLI. Instala y autenticate con Gemini CLI primero, luego `claudex-config auth login google` lee el token almacenado.
