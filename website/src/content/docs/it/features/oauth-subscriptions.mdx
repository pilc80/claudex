...
Non e necessario il passaggio `claudex-config auth login`. Se sei gia autenticato in Claude Code, funziona immediatamente.
...
1. `claudex-config auth login github` mostra un device code
...
Legge le credenziali dalla configurazione di Gemini CLI. Installa e autenticati con Gemini CLI prima, poi `claudex-config auth login google` legge il token archiviato.
