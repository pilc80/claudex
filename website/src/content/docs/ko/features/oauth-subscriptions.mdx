...
별도의 `claudex-config auth login` 단계가 필요 없습니다. Claude Code에 이미 로그인되어 있다면 즉시 작동합니다.
...
1. `claudex-config auth login github`이 디바이스 코드를 표시
...
Gemini CLI 설정에서 자격 증명을 읽습니다. Gemini CLI를 먼저 설치하고 인증한 후, `claudex-config auth login google`이 저장된 토큰을 읽습니다.
