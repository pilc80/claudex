...
Nenhum passo de `claudex-config auth login` e necessario. Se voce ja estiver logado no Claude Code, funciona imediatamente.
...
1. `claudex-config auth login github` exibe um device code
...
Le credenciais da configuracao do Gemini CLI. Instale e autentique com o Gemini CLI primeiro, e entao `claudex-config auth login google` le o token armazenado.
