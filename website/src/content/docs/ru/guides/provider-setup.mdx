...
<Aside type="note">
  OAuth-профили Claude используют специальный режим gateway auth. Claudex пропускает прокси и позволяет Claude Code использовать собственный OAuth напрямую. Отдельный шаг `claudex auth login` не требуется.
</Aside>
