...
无需 `claudex-config auth login` 步骤。如果你已登录 Claude Code，即可直接使用。
...
1. `claudex-config auth login github` 显示一个设备码
...
从 Gemini CLI 配置读取凭证。先安装并完成 Gemini CLI 认证，然后 `claudex-config auth login google` 读取已存储的 token。
